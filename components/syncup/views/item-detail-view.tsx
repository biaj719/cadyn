...
export function ItemDetailView({ itemId, itemType, onBack }: ItemDetailViewProps) {
  ...
  const userStatus = userTask?.status || "not-started";
  const isUserComplete = userStatus === "completed";
  const isUserInProgress = userStatus === "in-progress";

  // Get group statuses
  const groupStatuses = mockTrip.households.map(household => {
  ...
  const completeCount = groupStatuses.filter(g => g.status === "completed").length;
  const totalGroups = groupStatuses.length;
  const progressPercent = Math.round((completeCount / totalGroups) * 100);
    ...
    <div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className={`border-2 p-6 sm:p-8 ${
              ...
              <div className="flex items-start gap-4 mb-6">
                <div className={`rounded-xl p-3 ${
                  isUserComplete ? "bg-emerald-100" : "bg-muted"
                }`}>
                  <Icon className={`h-6 w-6 ${
                    isUserComplete ? "text-emerald-700" : "text-muted-foreground"
                  }`} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-lg font-semibold text-foreground">Your Status</h3>
              ...
              {!isUserComplete && (
                <div className="flex flex-wrap gap-3">
                  <Button className="gap-2">
                    {isUserInProgress ? "Continue booking" : "Get started"}
                  </Button>
          ...
          <div>
            <Card className="border-border/50 p-5 sticky top-6">
              <h3 className="font-semibold text-foreground mb-4 text-sm uppercase tracking-wide">Quick Reference</h3>
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    {isUserComplete && (
                      <>
                        ...
                        <span className="text-sm font-medium text-emerald-700">Booked</span>
                      </>
                    )}
                    {isUserInProgress && (
                      <>
                ...
                <div className="pt-2 border-t border-border/50">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide mb-2">Group Progress</p>
                  <p className="text-2xl font-bold text-foreground mb-1">{completeCount}/{totalGroups}</p>
                  <p className="text-xs text-muted-foreground">households booked</p>
                </div>

                {!isUserComplete && (
                  <div className="pt-2 border-t border-border/50">
                    <p className="text-xs text-muted-foreground uppercase tracking-wide mb-2">Impact</p>
                    <p className="text-sm text-muted-foreground">Completing this helps the group stay coordinated and funded.</p>
                  </div>
                )}
              </div>
            </Card>
